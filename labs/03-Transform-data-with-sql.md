...
5. After the repo has been cloned, enter the following commands to change to the folder for this exercise and run the **setup.ps1** script it contains:
    ...
    ```
    cd dp-203/labs/alldata/lab3
    ./setup.ps1
    ```

6. When prompted, type your storage account, resource group and default synapse filesystem names.
![Script parameters picture](/labs/pics/3-0-script-params.png)


8. Wait for the script to complete - this typically takes around 1 minute
...
1. After the script has completed, in the Azure portal, go to the  Synapse workspace.
...
5. Expand your storage account and verify that it contains a file system container named **synapsedata**.
6. Select the **synapsedata** container, and note that it contains a folder named **sales**. This folder contains the data files you are going to query.
...
4. Review the SQL code that has been generated, which should be similar to this:
    ...
    FROM
        OPENROWSET(
            BULK 'https://xxxxxxx.dfs.core.windows.net/files/sales/csv/**',
            FORMAT = 'CSV',
            PARSER_VERSION='2.0'
        ) AS [result]
    ```
...
2. In the new script pane, add the following code (replacing *xxxxxxx* with the name of your data lake storage account) to create a new database and add an external data source to it.
    ...
    CREATE EXTERNAL DATA SOURCE sales_data WITH (
        LOCATION = 'https://xxxxxxx.dfs.core.windows.net/synapsedata/'
    );
    GO;
